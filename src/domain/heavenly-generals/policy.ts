...
export type HeavenlyGeneralsPolicyErrorKind = "noble-branch-lookup-failed" | "noble-palace-not-unique" | "invalid-direction" | "placement-incomplete";
export class HeavenlyGeneralsPolicyError extends Error {
  constructor(public readonly kind: HeavenlyGeneralsPolicyErrorKind, message: string) {
    super(message);
    this.name = "HeavenlyGeneralsPolicyError";
  }
}
...
export function deriveHeavenlyGenerals(dayStem: HeavenlyStem, divinationHour: EarthlyBranch, plate: HeavenEarthResult): HeavenlyGeneralsResult {
  ...
  const nobleTable = NOBLE_BRANCHES[dayStem] as Record<NobleDayNight, EarthlyBranch> | undefined;
  const nobleHeaven = nobleTable?.[dayNight];
  if (!nobleHeaven) throw new HeavenlyGeneralsPolicyError("noble-branch-lookup-failed", `日干${dayStem}贵人支查取失败`);
  const matching = plate.palaces.filter(({ heaven }) => heaven === nobleHeaven);
  if (matching.length !== 1) throw new HeavenlyGeneralsPolicyError("noble-palace-not-unique", `贵人天盘支${nobleHeaven}所临地盘宫不唯一`);
  const nobleEarth = matching[0].earth;
  const isForward = (FORWARD_NOBLE_EARTHS as readonly EarthlyBranch[]).includes(nobleEarth);
  if (!isForward && !(EARTHLY_BRANCHES as readonly EarthlyBranch[]).includes(nobleEarth)) {
    throw new HeavenlyGeneralsPolicyError("invalid-direction", `贵人落宫${nobleEarth}无法确定布将方向`);
  }
  if (!Number.isInteger(plate.offset) || plate.offset < 0 || plate.offset > 11 || plate.palaces.length !== 12 || !plate.palaces.every(({ earth, heaven }, index) => earth === EARTHLY_BRANCHES[index] && heaven === EARTHLY_BRANCHES[(index + plate.offset) % 12])) {
    throw new HeavenlyGeneralsPolicyError("placement-incomplete", "天地盘十二支布列无效");
  }
  const direction: GeneralDirection = isForward ? "forward" : "reverse";
  ...
  const placements = GENERAL_ORDER.map((general, order) => {
    const earth = EARTHLY_BRANCHES[(nobleIndex + movement * order + 12) % 12];
    const palace = plate.palaces.find((item) => item.earth === earth);
    if (!palace) throw new HeavenlyGeneralsPolicyError("placement-incomplete", `天地盘缺少${earth}宫`);
    return { order, general, earth, heaven: palace.heaven, evidenceId: `hg-${String(order + 5).padStart(2, "0")}` };
  });
